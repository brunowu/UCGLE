...
PetscErrorCode LSInitialGuess(com_lsa * com, KSP ksp)
{
  ...
  PetscInt size_data,initial_guess_power,latency,hang,timing;
  PetscInt nols;
  PetscBool flag;
  int i,j, size;
  ...
  ierr=PetscOptionsGetInt(NULL,PETSC_NULL,"-initial_guess_power",&initial_guess_power,&flag);CHKERRQ(ierr);
  if(!flag) initial_guess_power=LS_POWER;
...
//	PetscPrintf(PETSC_COMM_WORLD,"\n\nread\n\n");
  ...
  for(j=0;j<initial_guess_power;j++){
    /* r0 = b-Ax*/
